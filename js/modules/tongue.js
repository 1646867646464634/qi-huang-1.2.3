// ===== 岐黄·辅助诊疗系统 - 舌诊模块（问诊式表单化） =====
// 原"上传照片→AI模拟分析"改为结构化问诊表单：按《中医诊断学》望诊要素逐项选择，
// 由 DiagnosisEngine 基于 syndromesDatabase 加权打分给出辨证参考。纯前端、可离线。
class TongueModule {
    constructor() {
        this.selection = {};
            ...
            <div class="module-page tongue-page">
                ...
                <div class="card" style="margin-bottom: var(--space-lg);">
                    ...
                    <ul class="guide-list" style="font-size: var(--text-sm); color: var(--color-ink-light); padding-left: var(--space-lg); line-height: 2;">
                        <li>舌色：观察舌体整体颜色，注意区分淡红、淡白、红、绛红、青紫</li>
                        <li>舌形：观察舌体大小厚薄，注意胖大、瘦薄、齿痕、裂纹、点刺</li>
                        <li>苔色：观察舌苔颜色（白苔主表寒湿，黄苔主里热，灰黑苔为重证）</li>
                        <li>苔质：观察舌苔质地（薄厚腻腐燥润剥落），判断邪气深浅与津液盈亏</li>
                ...
                <div class="card" style="margin-bottom: var(--space-lg);">
                    ...
                    <div class="upload-zone" id="tongueUploadZone" role="button" tabindex="0" aria-label="上传舌象图片">
                        <input type="file" id="tongueUploadInput" accept="image/jpeg,image/jpg,image/png" hidden>
                        <div class="upload-icon">📷</div>
                        <div class="upload-text">点击或拖拽舌象照片上传，AI 自动识别并回填下方表单</div>
                    ...
                    <div class="analyzing-spinner" id="tongueAnalyzingSpinner" style="display:none;">
                ...
                <div id="tongueResultContainer" class="result-container" style="display:none;"></div>

                <div class="disclaimer" style="margin-top: var(--space-xl);">
                    <strong>免责声明：</strong>舌诊问诊分析仅作为学习与辅助参考，不能替代专业中医师的诊断。
                    舌诊是中医四诊"望诊"的重要组成部分，需结合问诊、闻诊、切诊进行综合判断。
                </div>
            </div>
        `;
    ...
    renderForm() {
        const rules = (typeof TongueRules !== 'undefined') ? TongueRules : null;
        if (!rules) return '<p>舌诊规则数据未加载</p>';
        ...
        return `
                    ...
                    <div class="form-option-grid" style="display:flex; flex-wrap:wrap; gap:8px;">
                        ${field.options.map(opt => `
                            <label class="form-option-chip" style="display:inline-flex; align-items:center; gap:6px; padding:8px 14px; border:1px solid var(--color-line,#e2d9cc); border-radius:20px; cursor:pointer; font-size:var(--text-sm); background:var(--color-card,#FBF8F4); transition:all .15s;">
                                <input type="${field.multi ? 'checkbox' : 'radio'}" name="tongue_${field.key}" value="${opt.value}" style="accent-color:var(--color-vermillion,#C04040);">
                                <span>${opt.label}</span>
            ...
            <div style="display:flex; gap:10px; margin-top: var(--space-md);">
                <button class="btn btn-primary" id="tongueDiagnoseBtn">开始辨证</button>
                <button class="btn btn-outline" id="tongueResetBtn">重置选择</button>
            </div>
        `;
    }

    bindEvents(container) {
        const diagnoseBtn = container.querySelector('#tongueDiagnoseBtn');
        const resetBtn = container.querySelector('#tongueResetBtn');
        ...
        if (resetBtn) {
            resetBtn.addEventListener('click', () => {
                ...
                const rc = container.querySelector('#tongueResultContainer');
                if (rc) { rc.style.display = 'none'; rc.innerHTML = ''; }
                this.clearUpload(container, 'tongue');
            });
        }

        this.bindUploadEvents(container, 'tongue', (typeof TongueRules !== 'undefined') ? TongueRules : null);
    }
    ...
    performDiagnosis(container) {
        const rules = (typeof TongueRules !== 'undefined') ? TongueRules : null;
        if (!rules) { Toast.show('舌诊规则数据未加载', 'warning'); return; }

        // 收集选择
        const selection = {};
        rules.fields.forEach(field => {
            const inputs = container.querySelectorAll(`input[name="tongue_${field.key}"]:checked`);
        ...
        if (totalSelected < 2) {
            Toast.show('请至少选择两项舌象特征（如舌色 + 苔色）', 'warning');
            return;
        }
    ...
    renderResults(container, results, selection, rules) {
        const resultContainer = container.querySelector('#tongueResultContainer');
            ...
            resultContainer.innerHTML = `
                <div class="empty-state card" style="text-align:center; padding: var(--space-2xl) var(--space-lg);">
                    ...
                    <p style="color: var(--color-ink-pale); font-size: var(--text-sm);">
                        您选择的舌象组合较难对应到知识库中的明确证型，建议核对观察要点后重试，或咨询专业医师。
                    </p>
                </div>
            `;
        ...
        resultContainer.innerHTML = `
            <div class="card">
                ...
                ${results.map((r, i) => `
                    <div class="result-card" style="border:1px solid var(--color-line,#e2d9cc); border-radius:10px; margin-bottom: var(--space-md); overflow:hidden;">
                        <div class="result-header" style="padding:14px 18px; background:rgba(192,64,64,.05); cursor:pointer; display:flex; justify-content:space-between; align-items:center;">
                            <div>
                                <span class="tag tag-syndrome-link" data-syndrome-search="${this.escapeAttr(r.syndrome.name)}" style="font-size:16px; font-weight:600; color:var(--color-vermillion-dark);">${r.syndrome.name}</span>
                        ...
                        <div class="result-body" style="padding:14px 18px; display:block;">
            ...
            <div style="display:flex;gap:10px;margin-top:var(--space-md);">
                <button class="btn btn-primary" id="tongueSaveRecordBtn">💾 保存诊断记录</button>
            </div>
        `;
        resultContainer.style.display = 'block';

        // 保存诊断记录
        const saveBtn = resultContainer.querySelector('#tongueSaveRecordBtn');
        if (saveBtn) {
            saveBtn.addEventListener('click', () => {
                if (typeof Records !== 'undefined' && Records.save) {
                    Records.save({
                        type: 'tongue',
                        input: { tongue: this.selection, ai: this.aiSelection || undefined },
...
if (typeof window !== 'undefined') {
    window.TongueModule = TongueModule;
}
